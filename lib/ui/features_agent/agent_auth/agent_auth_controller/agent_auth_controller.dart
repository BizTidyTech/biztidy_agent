...
class AgentAuthController extends GetxController {
  ...
  Future<void> attemptSignIn(BuildContext context) async {
    ...
    try {
      ...
      // Use Firestore emailVerified (OTP-based), not Firebase Auth link flag
...
class AgentOtpController extends GetxController {
  ...
  /// Sends a 6-digit OTP to [email] using the email_otp package.
  ...
  Future<void> verifyOtp({
    ...
    isVerifying = true; invalidOtp = false; errMessage = ''; update();
    ...
    // Persist verified state to Firestore
  ...
  Future<void> resendOtp(BuildContext context) async {
    ...
    isResending = true; invalidOtp = false; update();
...
class AgentSignupController extends GetxController {
  ...
  bool showLoading = false;
  bool isObscured = true;
  bool isPickingPassport = false;
  bool isPickingId = false;
  String errMessage = '';
  ...
  Future<void> pickPassportPhoto() async {
    ...
    try {
      ...
      if (picked != null) passportPhoto = File(picked.path);
    } catch (e) {
      Fluttertoast.showToast(
  ...
  Future<void> pickIdDocument() async {
    ...
    try {
      ...
      if (picked != null) idDocument = File(picked.path);
    } catch (e) {
      Fluttertoast.showToast(
        msg: 'Could not open gallery. Please check app permissions.',
        backgroundColor: Colors.red,
